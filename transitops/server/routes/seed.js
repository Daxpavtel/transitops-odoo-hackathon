...
router.get('/', async (req, res) => {
  try {
    ...
    return res.json({
      success: true,
      message: "Database seeded successfully into MongoDB Atlas directly via Vercel!",
      seeded: {
        users: seedUsers.map(u => ({ email: u.email, role: u.role })),
        vehiclesCount: vehicles.length,
        driversCount: drivers.length,
        tripsCount: seededTrips.length
      },
      credentialsNote: "All accounts use password: Password@123"
    });
  } catch (error) {
