...
Deploy from the repository root. The root `vercel.json` builds the Vite client, routes `/api/*` requests to the Express serverless function, and sends all other paths to the SPA entry point. This prevents a repository-root deployment from missing the nested application configuration.

Use these Vercel Build and Development settings:
...
Configure these environment variables in the Vercel dashboard for Production (and Preview if required): `MONGO_URI`, `JWT_SECRET`, `NODE_ENV=production`, and `CLIENT_ORIGIN=https://<your-project>.vercel.app`. Do not commit real credentials or a production JWT secret.

The existing `transitops/vercel.json` remains available for Vercel projects whose Root Directory is intentionally set to `transitops`.
